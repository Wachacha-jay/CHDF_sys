// Run migration 023 via Node + mysql2
const mysql = require('mysql2/promise');
const fs = require('fs');
const path = require('path');
require('dotenv').config({ path: path.join(__dirname, 'server/.env') });

async function run() {
  const conn = await mysql.createConnection({
    host: process.env.DB_HOST || 'localhost',
    user: process.env.DB_USER || 'root',
    password: process.env.DB_PASSWORD || '',
    database: process.env.DB_NAME || 'business_management',
    multipleStatements: true,
    connectTimeout: 5000
  });

  console.log('Connected to DB:', process.env.DB_NAME || 'business_management');

  const sql = fs.readFileSync(
    path.join(__dirname, 'database/migrations/023_payroll_sacco_and_individual_payments.sql'),
    'utf8'
  );

  const statements = sql
    .split(';')
    .map(s => s.trim())
    .filter(s => s && !s.startsWith('--'));

  let ok = 0, errs = 0;
  for (const stmt of statements) {
    try {
      await conn.query(stmt);
      console.log('  OK:', stmt.substring(0, 80).replace(/\n/g, ' '));
      ok++;
    } catch (e) {
      console.warn('  SKIP/ERR:', e.message.substring(0, 100));
      errs++;
    }
  }

  await conn.end();
  console.log(`\nDone. ${ok} statements OK, ${errs} skipped/errored.`);
  process.exit(0);
}

run().catch(e => { console.error('Migration error:', e.message); process.exit(1); });
